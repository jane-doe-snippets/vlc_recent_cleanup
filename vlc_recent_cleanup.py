...
def drop_files_by_user_func(*, plist: dict, should_drop_func: ShouldDropFileFunc) -> None:
    # process list
...
def drop_files_by_ext(*, plist: dict, extensions: Set[str]) -> None:
    ...
    drop_files_by_user_func(plist=plist, should_drop_func=should_drop)
...
def drop_files_inside_dirs(*, plist: dict, exclude_dirs: Set[str]) -> None:
...
def read_cli_config(args: List[str]) -> Config:
    ...
    return Config(drop_exts=set(result.drop_exts), drop_dirs=set(result.drop_dirs))
...
def main(args: List[str]) -> None:
    ...
    drop_files_by_ext(plist=plist, extensions=config.drop_exts)
    drop_files_inside_dirs(plist=plist, exclude_dirs=config.drop_dirs)
